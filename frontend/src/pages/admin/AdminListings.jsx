...
const AdminListings = () => {
  ...
  const statusFiltered = filter === 'ALL' ? listings : listings.filter(l => l.status === filter);
    ...
    <DashboardLayout>
      ...
      <div className="mb-8 flex flex-col sm:flex-row sm:items-end justify-between gap-4" style={{ animation: 'fadeUp .4s ease both' }}>
        <div>
          ...
          <h1 className="text-3xl font-black text-white">Manage Listings</h1>
        </div>
        <input
      ...
      {listings.length > 0 && (
        <div className="grid grid-cols-3 gap-4 mb-6" style={{ animation: 'fadeUp .4s .05s ease both', opacity: 0, animationFillMode: 'forwards' }}>
          {[
            ...
            { label: 'Available', value: listings.filter(l => l.status === 'AVAILABLE').length,   color: 'text-emerald-400' },
            { label: 'Filled',    value: listings.filter(l => l.status === 'FILLED').length,      color: 'text-red-400' },
          ].map(s => (
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500">
                      Owner: <span className="text-slate-400">{listing.owner?.name}</span>
                      <span className="text-slate-600 mx-2">·</span>
